...
import com.dmm.search.dao.SkuEsMapper;
import com.dmm.search.feign.SkuFeign;
...
public class SkuServiceImpl implements SkuService {
    ...
    @Override
    public void importEs(Long spuId) {
        //1.调用 goods微服务的fegin 查询 符合条件的sku的数据
        Result<List<Sku>> skuResult = skuFeign.findByStatusAndSpuId(spuId, StatusCode.Sku_on_sale);
        List<Sku> data = skuResult.getData();//sku的列表
        skuEsMapper.saveAll(data);
    }

    @Override
    public void exportEs(Long spuId) {
        Sku sku = new Sku();
        sku.setSpuId(spuId);
        skuEsMapper.delete(sku);
    }
    ...
    public Map search(Map<String, String> searchMap) {
        ...
        nativeSearchQueryBuilder.addAggregation
                (AggregationBuilders.terms("skuBrandGroup").field("brandName").size(100));

        //4.3 商品的规格的列表展示   按照商品的规格的字段spec 进行分组
        //规则 要求 字段 是一个keyword类型的  spec.keyword
        nativeSearchQueryBuilder.
                addAggregation(AggregationBuilders.terms("skuSpecGroup").field("spec.keyword").size(500));
        ...
        //从多个字段中搜索数据
        nativeSearchQueryBuilder.
                withQuery(QueryBuilders.multiMatchQuery(keywords, "name", "categoryName", "brandName"));
        ...
        //6.执行查询
        SearchHits<Sku> skuSearchHits = elasticsearchTemplate.search(nativeSearchQuery, Sku.class);


        // 6.2 获取聚合分组结果  获取商品分类的列表数据
        Aggregations aggregations = skuSearchHits.getAggregations();
        ParsedStringTerms skuCategoryGroup = aggregations.get("skuCategoryGroup");
        ...
        ParsedStringTerms skuBrandGroup = aggregations.get("skuBrandGroup");

        List<String> brandList = getStringsList(skuBrandGroup);
        //6.4 获取 规格的分组结果 列表数据map
        ParsedStringTerms skuSpecGroup = aggregations.get("skuSpecgroup");
        Map<String, Set<String>> specMap = getStringSetMap(skuSpecGroup);

        //7.获取结果  返回map

        List<Sku> content = getContent(skuSearchHits);//当前的页的集合
                long totalHits = skuSearchHits.getTotalHits();//总记录数
    ...
    private List<Sku> getContent(SearchHits searchHits) {
        List<Sku> skuList = new ArrayList<>();
        List<SearchHit<Sku>> searchHitList=searchHits.getSearchHits();
        for (SearchHit<Sku> searchHit : searchHitList) {
            skuList.add(searchHit.getContent());
        }

       return skuList;
    }

    private Map<String, Set<String>> getStringSetMap(ParsedStringTerms parsedStringTerms) {
        //key :规格的名称
     ...
     * 获取分组结果
     */
    private List<String> getStringsList(ParsedStringTerms parsedStringTerms) {
